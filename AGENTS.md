...
`components/faculty/FacultySectionList.tsx` is a thin dispatcher over a `source` prop: `"live"` (the portal, via `features/teacher-profile/`) or `"demo"` (the admin's per-teacher preview workspace at `/admin/faculty/[id]/*`, via `FacultyProfileProvider`'s in-memory state). Both render the same `components/faculty/section-list/SectionListView.tsx`. The `/teachers/profile/*` session-scoped API this section describes is still not addressable by an admin for an arbitrary teacher id — but the admin dashboard now has its own, separate CRUD for a teacher's education/experience/membership/awards/publications via `/teachers/admin/info/*`, registered in `services/endpoints.ts` and live at `/admin/people/{education,experience,membership,awards,teacher-publication}`. The per-teacher preview workspace at `/admin/faculty/[id]/*` simply hasn't been rewired from `FacultyProfileProvider`'s demo state onto that admin CRUD yet.
